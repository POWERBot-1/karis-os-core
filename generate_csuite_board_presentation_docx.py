...
| `KARIS_OS_DEFAULT` | **KARIS OS™ Enterprise Platform** | `#0B2545` (KARIS Navy) | Flagship 23-vertical deployment with full C-suite BI executive aggregation across all 6 business domains including KARIS BorderX Customs Engine. |
...
"""
    ...
    profiles_data = [
        ...
        ("PALPLUS_GLOBAL_CHECKOUT_OS", "PalPlus Universal Commerce & Checkout OS", "#2563EB (PalPlus Blue)", "Universal hosted checkout URLs (`link.palpluss.com/6e8de0bc...`), prediction escrow checkouts (`POWER BOT X`), and real estate dividends."),
        ("KARIS_OS_DEFAULT", "KARIS OS™ Enterprise Platform", "#0B2545 (KARIS Navy)", "Flagship 23-vertical deployment across 58 Sections with full C-suite BI executive aggregation across all 6 business domains including KARIS BorderX Customs Engine.")
    ]
    ...
    p3 = doc.add_paragraph(
        "The platform has undergone rigorous, automated verification right inside our cloud container (`/home/user/karis-os-core/`), proving exact C-suite benchmarks:\n"
        "• High-Throughput Concurrency Throughput: 2,484.2+ operations / second across 16 concurrent threads (`ThreadPoolExecutor`).\n"
        "• Automated Integration Test Verification: 100% PASS (`74 / 74 multi-tenant tests passed in 0.86s`) across 26 Pytest suites covering 23 Industry Verticals across 58 Sections.\n"
        "• Cryptographic Hash Immutability (`Rule 9`): VERIFIED_CLEAN double-entry audit anchor (`last_hash: 42a1f07f9c...`). Zero data corruption under chaos fault injection (`DLQ Self-Healing Engine`).\n"
        "• Turnkey One-Click Cloud Deployment: Complete Docker multi-stage build, `k8s/deployment.yaml` manifest, and one-click blueprints (`render.yaml`, `railway.json`)."
    )
